...
local function get_active_preset()
	...
	for k, v in pairs(config) do
		if merged[k] == nil or config.quality == "custom" then
			merged[k] = v
		end
	end
...
local function make_clip()
	...
	if p.scale and p.scale ~= "" then
		...
		table.insert(args, "scale=" .. p.scale)
	end
	...
	mp.command_native_async(
		{ name = "subprocess", args = args, capture_stdout = true, capture_stderr = true },
		function() end
	)
...
mp.add_key_binding("q", "cycle-quality", function()
	local idx
	for i, v in ipairs(preset_order) do
		if v == config.quality then
			idx = i
			break
		end
	end
