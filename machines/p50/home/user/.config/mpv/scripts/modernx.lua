...
function checktitle()
	...
	if
		mp.get_property("filtered-metadata/by-key/Album_Artist") and mp.get_property("filtered-metadata/by-key/Artist")
	then
		if
	...
	if is_url(path) and path or nil then
		...
		if path:find("youtu%.?be") and (user_opts.show_description or user_opts.title_youtube_stats) then
			...
			if match then
				...
				if videoID then
					exec_async(
						{ "curl", "https://returnyoutubedislikeapi.com/votes?videoId=" .. videoID },
						process_dislikes
					)
				else
...
function process_vid_stats(success, result, error)
	...
	state.localDescriptionClick = mp.get_property("media-title", "")
		.. "\\N────────────────────\\N"
		.. string.gsub(string.gsub(result.stdout, "\r", "\\N") .. state.dislikes, "\n", "\\N")
		.. "\\N────────────────────\\N"
		.. state.ytdescription
	...
	if false then
		state.localDescriptionClick = state.localDescriptionClick
			...
			:gsub("Likes:", icons.emoticon.like)
			:gsub("Dislikes:", icons.emoticon.dislike) -- replace with icons
	end
...
layouts["original"] = function()
	...
	if
		(user_opts.window_top_bar == "yes" or not state.border or not state.title_bar or state.fullscreen)
		and top_titlebar
	then
	...
	if (state.localDescription ~= nil or state.is_URL) and user_opts.show_description then
		geo = {
			x = 25,
			y = refY - 117 + (user_opts.seekbar_between_timers and 25 or 0),
			an = 1,
			w = osc_geo.w - 50,
			h = 19,
		}
	...
	if jump_buttons then
		...
		lo.style = (user_opts.jump_icon_number and icons.jumpicons[user_opts.jump_amount] ~= nil)
				and osc_styles.control_2
	...
	lo.geometry =
		{ x = 25, y = refY - 84, an = 7, w = 35 + (state.tc_ms and 30 or 0) + (show_hours and 20 or 0), h = 20 }
	...
	then
		...
		if h > 0 then
			local subpos = math.floor(
				(
					osc_param.playresy
					- (user_opts.raise_subtitle_amount - (user_opts.seekbar_between_timers and 25 or 0))
				)
					/ osc_param.playresy
					* 100
			)
	...
	if not (state.anitype == nil) then
		...
		if now < state.anistart + (user_opts.fade_duration / 1000) then
			if state.anitype == "in" then --fade in
				osc_visible(true)
				state.animation =
					scale_value(state.anistart, (state.anistart + (user_opts.fade_duration / 1000)), 255, 0, now)
			elseif state.anitype == "out" then --fade out
				state.animation =
					scale_value(state.anistart, (state.anistart + (user_opts.fade_duration / 1000)), 0, 255, now)
			end
		else
		...
		then
			if user_opts.bottom_hover then -- if enabled, only show osc if mouse is hovering at the bottom of the screen (where the UI elements are)
				...
				if
					...
					or (
							user_opts.window_top_bar == "yes"
							or not state.border
							or not state.title_bar
							or state.fullscreen
						)
